...
def get_conf(config_name):
    ...
    if config_interface != {}:
        ...
        result[
            "message"
        ] = "I cannot find this configuration. <br> Please refresh and try again"
    config.clear()
    return jsonify(result)
            ...
            return jsonify(
                {
                    ...
                    "message": str(exc.output.strip().decode("utf-8")),
                }
            )
    elif status == "stopped":
        try:
...
def addConfigurationPortCheck():
    ...
    required = ["port"]
    if checkJSONAllParameter(required, data):
        returnData = api.manageConfiguration.PortCheck(
            api.manageConfiguration, data, get_conf_list()
        )
    else:
        return jsonify(api.notEnoughParameter)
    return jsonify(returnData)
...
def addConfigurationNameCheck():
    ...
    required = ["name"]
    if checkJSONAllParameter(required, data):
        returnData = api.manageConfiguration.NameCheck(
            api.manageConfiguration, data, get_conf_list()
        )
    else:
        return jsonify(api.notEnoughParameter)
...
def addConfiguration():
    ...
    nameCheck = api.manageConfiguration.NameCheck(
        api.manageConfiguration, {"name": data["addConfigurationName"]}, config
    )
    if not nameCheck["status"]:
        return nameCheck

    portCheck = api.manageConfiguration.PortCheck(
        api.manageConfiguration, {"port": data["addConfigurationListenPort"]}, config
    )
    if not portCheck["status"]:
        return portCheck

    addressCheck = api.manageConfiguration.AddressCheck(
        api.manageConfiguration, {"address": data["addConfigurationAddress"]}
    )
    if not addressCheck["status"]:
        return addressCheck

    returnData = api.manageConfiguration.addConfiguration(
        api.manageConfiguration, data, config, WG_CONF_PATH
    )
    return jsonify(returnData)
...
def deleteConfiguration():
    ...
    if not checkJSONAllParameter(required, data):
        return jsonify(api.notEnoughParameter)
    returnData = api.manageConfiguration.deleteConfiguration(
        api.manageConfiguration, data, get_conf_list(), g, WG_CONF_PATH
    )
    return returnData
...
def getConfigurationInfo():
    ...
    if not checkJSONAllParameter(required, data):
        ...
        return api.manageConfiguration.getConfigurationInfo(
            api.manageConfiguration, data["configName"], WG_CONF_PATH
        )
...
def setTheme():
    ...
    if not checkJSONAllParameter(required, data):
        ...
        return api.settings.setTheme(
            api.settings, data["theme"], get_dashboard_conf(), set_dashboard_conf
        )
    ...
    try:
        ...
        while True:
            ...
            for i in conf:
                ...
                try:
                    data_usage = subprocess.check_output(
                        f"wg show {config_name} transfer",
                        shell=True,
                        stderr=subprocess.STDOUT,
                    )
                    data_usage = data_usage.decode("UTF-8").split("\n")
                    ...
                    for i in range(len(data_usage)):
                        ...
                        if len(cur_i) > 0:
                            total_sent = cur_i[0][1]
                            total_receive = cur_i[0][0]
                            cur_total_sent = round(
                                int(data_usage[i][2]) / (1024**3), 4
                            )
                            cur_total_receive = round(
                                int(data_usage[i][1]) / (1024**3), 4
                            )
                            cumulative_receive = cur_i[0][2] + total_receive
                            cumulative_sent = cur_i[0][3] + total_sent
                            if (
